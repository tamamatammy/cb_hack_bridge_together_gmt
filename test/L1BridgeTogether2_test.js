...
describe("L1BridgeTogether2", function () {
  it("should send the funds", async function () {
 
    const L1BridgeTogether2_contract = await ethers.getContractFactory("L1BridgeTogether2");
    const l2Address = "0x70997970C51812dc3A010C7d01b50e0d17dc79C8";
    const userAddress = "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266";
    
    const instance = await L1BridgeTogether2_contract.deploy(l2Address);
    ...
    expect(totalBalance).to.equal(ethers.utils.parseEther("0"));
    ...
    addressesCount = await instance.getCount();
    totalBalance = await instance.getTotalBalance();
    expect(addressesCount).to.equal(3);
    expect(totalBalance).to.equal(ethers.utils.parseEther("0"));
    // contractBalance = await ethers.provider.getBalance(instance.address)
